...
CASE 
    WHEN s4_status IS NULL THEN {'s4': null, 'ai2': ai2_status}
    WHEN ai2_status IS NULL THEN {'s4': s4_status, 'ai2': null}
    WHEN s4_status LIKE 'OPER%' AND ai2_status = 'OPERATIONAL' THEN {'s4': s4_status, 'ai2': '**'}
    WHEN s4_status LIKE 'NOP%' AND ai2_status = 'NON OPERATIONAL' THEN {'s4': s4_status, 'ai2': '**'}
    WHEN s4_status LIKE 'DCOM%' AND ai2_status = 'DECOMMISSIONED' THEN {'s4': s4_status, 'ai2': '**'}
    WHEN s4_status LIKE 'OOS%' AND ai2_status = 'OUT OF SERVICE' THEN {'s4': s4_status, 'ai2': '**'}
    WHEN s4_status LIKE 'DISP%' AND ai2_status = 'DISPOSED OF' THEN {'s4': s4_status, 'ai2': '**'}
    WHEN s4_status LIKE 'TBCM%' AND ai2_status = 'TO BE COMMISSIONED' THEN {'s4': s4_status, 'ai2': '**'}
    ELSE {'s4': s4_status, 'ai2': ai2_status}
END;


CREATE OR REPLACE TEMPORARY MACRO compare_manufacturer(s4_manuf, ai2_manuf) AS 
CASE 
    WHEN s4_manuf IS NULL THEN {'s4': null, 'ai2': ai2_manuf}
    WHEN ai2_manuf IS NULL THEN {'s4': s4_manuf, 'ai2': null}
    WHEN s4_manuf = ai2_manuf THEN {'s4': s4_manuf, 'ai2': '**'}
    ELSE {'s4': s4_manuf, 'ai2': ai2_manuf}
END;

CREATE OR REPLACE TEMPORARY MACRO compare_model(s4_model, ai2_model) AS 
CASE 
    WHEN s4_model IS NULL THEN {'s4': null, 'ai2': ai2_model}
    WHEN ai2_model IS NULL THEN {'s4': s4_model, 'ai2': null}
    WHEN s4_model = ai2_model THEN {'s4': s4_model, 'ai2': '**'}
    ELSE {'s4': s4_model, 'ai2': ai2_model}
END;

CREATE OR REPLACE TEMPORARY MACRO compare_specific_model(s4_specific_model, ai2_specific_model) AS 
CASE 
    WHEN s4_specific_model IS NULL THEN {'s4': null, 'ai2': ai2_specific_model}
    WHEN ai2_specific_model IS NULL THEN {'s4': s4_specific_model, 'ai2': null}
    WHEN s4_specific_model = ai2_specific_model THEN {'s4': s4_specific_model, 'ai2': '**'}
    ELSE {'s4': s4_specific_model, 'ai2': ai2_specific_model}
END;

CREATE OR REPLACE TEMPORARY MACRO compare_serial_number(s4_serial_number, ai2_serial_number) AS 
CASE 
    WHEN s4_serial_number IS NULL THEN {'s4': null, 'ai2': ai2_serial_number}
    WHEN ai2_serial_number IS NULL THEN {'s4': s4_serial_number, 'ai2': null}
    WHEN s4_serial_number = ai2_serial_number THEN {'s4': s4_serial_number, 'ai2': '**'}
    ELSE {'s4': s4_serial_number, 'ai2': ai2_serial_number}
END;

CREATE OR REPLACE TEMPORARY MACRO compare_pandi_tag(s4_pandi_tag, ai2_pandi_tag) AS 
CASE 
    WHEN s4_pandi_tag IS NULL THEN {'s4': null, 'ai2': ai2_pandi_tag}
    WHEN ai2_pandi_tag IS NULL THEN {'s4': s4_pandi_tag, 'ai2': null}
    WHEN s4_pandi_tag = ai2_pandi_tag THEN {'s4': s4_pandi_tag, 'ai2': '**'}
    ELSE {'s4': s4_pandi_tag, 'ai2': ai2_pandi_tag}
END;

CREATE OR REPLACE TEMPORARY MACRO compare_parent_ref(s4_parent_ref, ai2_parent_ref) AS 
CASE 
    WHEN s4_parent_ref IS NULL THEN {'s4': null, 'ai2': ai2_parent_ref}
    WHEN ai2_parent_ref IS NULL THEN {'s4': s4_parent_ref, 'ai2': null}
    WHEN s4_parent_ref = ai2_parent_ref THEN {'s4': s4_parent_ref, 'ai2': '**'}
    ELSE {'s4': s4_parent_ref, 'ai2': ai2_parent_ref}
END;
...
SELECT COLUMNS(c -> c NOT LIKE '__ans_%') FROM cte;
