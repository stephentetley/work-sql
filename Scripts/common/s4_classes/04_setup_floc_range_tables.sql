...
CREATE OR REPLACE TABLE s4_classlists_landing.floc_class_ranges (
...
CREATE OR REPLACE TABLE s4_classlists_landing.floc_char_ranges (
    class_name VARCHAR NOT NULL, 
...
DELETE FROM s4_classlists_landing.floc_class_ranges;

INSERT INTO s4_classlists_landing.floc_class_ranges BY NAME 
WITH cte1 AS (
SELECT 
...
FROM s4_classlists_landing.floc_classlist_file
...
WITH cte1 AS (
    ...
    SELECT max(row_ix) AS end_index FROM s4_classlists_landing.floc_classlist_file 
), cte3 AS (
SELECT 
    class_name, 
    descrip AS class_descrip, 
...
DELETE FROM s4_classlists_landing.floc_char_ranges;

INSERT INTO s4_classlists_landing.floc_char_ranges BY NAME
WITH cte1 AS (
SELECT 
...
JOIN s4_classlists_landing.floc_class_ranges t2 ON t.start_index >= t2.start_index AND t.start_index <= t2.end_index
)
SELECT * FROM cte2
...
FROM s4_classlists_landing.floc_classlist_file t
JOIN cte1 t1 ON t.row_ix = t1.start_index
JOIN s4_classlists_landing.floc_class_ranges t2 ON t.row_ix >= t2.start_index AND t.row_ix <= t2.end_index
)
