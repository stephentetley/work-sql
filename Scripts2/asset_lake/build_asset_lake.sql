...
-- Preliminaries: 
-- Must have asset_lake already attached and set as the default database 
-- (by `use`)


-- ztables

create schema if not exists ztables;

create or replace table ztables.eqobjl as
select * from read_parquet(
    getvariable('asset_lake_resources') || '/s4_ztables/ztable_eqobjl.parquet');

create or replace table ztables.flobjl as
select * from read_parquet(
    getvariable('asset_lake_resources') || '/s4_ztables/ztable_flobjl.parquet');

create or replace table ztables.flocdes as
select * from read_parquet(
    getvariable('asset_lake_resources') || '/s4_ztables/ztable_flocdes.parquet');

create or replace table ztables.manuf as
select * from read_parquet(
    getvariable('asset_lake_resources') || '/s4_ztables/ztable_manuf.parquet');

create or replace table ztables.objtype_manuf as
select * from read_parquet(
...
create or replace table site_mapping.site_mapping as
select * from read_parquet(
...
create or replace table ai2_classlists.ai2_equi_classes as
select * from read_parquet(
...
create or replace table s4_classlists.s4_equi_classes as
select * from read_parquet(
    getvariable('asset_lake_resources') || '/s4_classlists/s4_equi_classlist.parquet');

create or replace table s4_classlists.s4_equi_enums as
select * from read_parquet(
    getvariable('asset_lake_resources') || '/s4_classlists/s4_equi_classlist.parquet');

create or replace table s4_classlists.s4_floc_classes as
select * from read_parquet(
    getvariable('asset_lake_resources') || '/s4_classlists/s4_floc_classlist.parquet');

create or replace table s4_classlists.s4_floc_enums as
select * from read_parquet(
    getvariable('asset_lake_resources') || '/s4_classlists/s4_floc_classlist.parquet');
...
create or replace table ai2_masterdata.ai2_floc as
select * from read_parquet(
    getvariable('asset_lake_resources') || '/ai2_masterdata/ai2_masterdata_floc.parquet');

create or replace table ai2_masterdata.ai2_equi as
select * from read_parquet(
