...
cte1 AS (
    ...
    FROM cte1 t1
    LEFT JOIN asset_lake.ztables.flocdes t2 ON t2.object_type = t1.func
    LEFT JOIN asset_lake.ztables.flocdes t3 ON t3.object_type = t1.proc_grp
    LEFT JOIN asset_lake.ztables.flocdes t4 ON t4.object_type = t1.proc
)  
(SELECT  
    -- Level 1 (site)
    site AS funcloc,
